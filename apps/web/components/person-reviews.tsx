...
type PersonReviewsProps = {
  personId: string;
  canEdit?: boolean;
};

export function PersonReviews({ personId, canEdit }: PersonReviewsProps) {
  const trpc = useTRPC();
  const reviewsQuery = useQuery(trpc.performanceReview.getByPersonId.queryOptions({ personId }));
  ...
  if (!canEdit && reviews.length === 0) return null;
