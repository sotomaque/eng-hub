...
test.describe("Roadmap", () => {
  ...
  test("clicking a milestone navigates to its detail page", async ({
    page,
  }) => {
    await page.goto("/projects/proj-alpha/roadmap");
    await expect(page.getByText("MVP Launch")).toBeVisible({ timeout: 15_000 });
    ...
    await expect(
      page.getByRole("heading", { name: "MVP Launch" }),
    ).toBeVisible();
    await expect(
  ...
  test("clicking a quarterly goal navigates to its detail page", async ({
    page,
  }) => {
    await page.goto("/projects/proj-alpha/roadmap");
    await expect(page.getByText("Test Coverage 80%")).toBeVisible({
    ...
    await expect(
      page.getByRole("heading", { name: "Test Coverage 80%" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Key Results" }),
    ).toBeVisible();
